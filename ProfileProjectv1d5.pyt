...
class Projector(object):
    ...
    def execute(self, parameters, messages):
        ...
        arcpy.CreateFeatureDataset_management(arcpy.env.scratchWorkspace, "MilesSplit", "Z:/Team/Crow/ArcTools/HardCodedPolygons/UTM11N.prj")
        arcpy.CreateFeatureDataset_management(arcpy.env.scratchWorkspace, "PointsSplit", "Z:/Team/Crow/ArcTools/HardCodedPolygons/UTM11N.prj")

        scratch=arcpy.env.scratchWorkspace

        #Split Points of Interest into zones
        messages.addMessage("Splitting points based on zones ...")
        arcpy.Split_analysis(in_features=toProjectFile,split_features=zonesFile,split_field="Name",out_workspace=scratch+"/PointsSplit",cluster_tolerance="#")
        ...
        for stuff in pointsList:
            arcpy.Rename_management(str(stuff), str(stuff)+"_pt")

        #Split Miles into zones
        messages.addMessage("Splitting miles based on zones ...")
        arcpy.Split_analysis(in_features=milesFile,split_features=zonesFile,split_field="Name",out_workspace=scratch+"/MilesSplit",cluster_tolerance="#")
        ...
        for junk in milesList:
            arcpy.Rename_management(str(junk), str(junk)+"_mi")
       ...
       #Find zones with points for projection
        ...
        for item in zonesList:
            if arcpy.Exists(scratch+"/PointsSplit/"+item) and arcpy.Exists(scratch+"/MilesSplit/"+item[0:-3]+"_mi"):
                messages.addMessage("Working on zone: "+str(item))
                messages.addMessage("Finding closest points ...")
                arcpy.Near_analysis(in_features=scratch+"/PointsSplit/"+item,near_features=scratch+"/MilesSplit/"+item[0:-3]+"_mi",search_radius="#",location="LOCATION",angle="ANGLE",method="PLANAR")
                messages.addMessage("Joining ...")
                arcpy.JoinField_management(in_data=scratch+"/PointsSplit/"+item,in_field="NEAR_FID",join_table=scratch+"/MilesSplit/"+item[0:-3]+"_mi",join_field="OBJECTID",fields="RM_final")
